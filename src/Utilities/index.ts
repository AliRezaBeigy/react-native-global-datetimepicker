...
export const getMonthDays = (
  ...
  return <DayInfo[]>Array.from({length: startDay + dayCount}, (_, i) => {
    ...
    return {
      value: new Date(year, month, i - startDay + 1, 12),
      label: persianNumber
...
export const getDateUTCString = (date?: Date): string | undefined => {
  if (date) return `${date.getFullYear()}/${date.getMonth()}/${date.getDate()}`;
